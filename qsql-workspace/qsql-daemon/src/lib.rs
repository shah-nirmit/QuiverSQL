...
pub struct DaemonState {
    ...
    pub database_sources: Arc<RwLock<HashMap<String, Arc<DatabaseRegistration>>>>,
    pub schema_cache: Arc<RwLock<HashMap<SchemaCacheKey, SchemaCacheEntry>>>,
...
pub struct DatabaseRegistration {
    ...
    pub dialect: Option<SqlDialectKind>,
...
impl DaemonState {
    pub fn new(engine: Arc<QsqlEngine>) -> Self {
        Self {
            engine,
    ...
    pub fn next_source_generation(&self) -> u64 {
...
pub async fn handle_request(req: RpcRequest, state: DaemonState) -> RpcResponse {
...
pub fn connector_query_error(error: qsql_connectors::ConnectorError) -> QueryError {
    QueryError {
        code: -32001,
...
pub fn cached_schema(state: &DaemonState, cache_key: &SchemaCacheKey) -> Option<SchemaRef> {
    let now = Instant::now();
...
pub fn cache_schema(state: &DaemonState, cache_key: SchemaCacheKey, schema: SchemaRef) {
...
pub fn clear_schema_cache_for_source(state: &DaemonState, alias: &str) {
    state
        .schema_cache
        .write()
        .unwrap()
...
pub fn find_database_registration(
    state: &DaemonState,
    alias: &str,
...
mod tests {
    ...
    fn make_sqlite_registration(gen: u64) -> Arc<DatabaseRegistration> {
        Arc::new(DatabaseRegistration {
            kind: SourceKind::Sqlite,
            generation: gen,
            connection_string: None,
    ...
    #[test]
    fn find_database_registration_exact_match() {
        let state = DaemonState::new(Arc::new(QsqlEngine::new()));
        let gen = state.next_source_generation();
        state
            .database_sources
            .write()
            .unwrap()
            .insert("MyDB".to_string(), make_sqlite_registration(gen));

        let result = find_database_registration(&state, "MyDB");
        assert!(result.is_some());
        assert_eq!(result.unwrap().0, "MyDB");
    }

    #[test]
    fn find_database_registration_case_insensitive_fallback() {
        let state = DaemonState::new(Arc::new(QsqlEngine::new()));
        let gen = state.next_source_generation();
        state
            .database_sources
        ...
        let lower = find_database_registration(&state, "mydb");
        assert!(lower.is_some(), "case-insensitive lookup should match");
        assert_eq!(lower.unwrap().0, "MyDB");

        let upper = find_database_registration(&state, "MYDB");
        assert!(upper.is_some());
    }

    #[test]
    fn find_database_registration_returns_none_for_unknown() {
        let state = DaemonState::new(Arc::new(QsqlEngine::new()));
        assert!(find_database_registration(&state, "ghost").is_none());
    }

    #[test]
    fn connector_query_error_always_returns_code_minus_32001() {
        use qsql_connectors::{ConnectorError, ConnectorErrorKind};

        let kinds = [
            ConnectorErrorKind::Connect,
            ConnectorErrorKind::Timeout,
            ConnectorErrorKind::Auth,
            ConnectorErrorKind::Sql,
        ...
        for kind in kinds {
            ...
            let details = qe.details.expect("details should be set");
            assert_eq!(details, format!("{kind:?}").to_lowercase());
        }
    }

    #[test]
    fn clear_schema_cache_removes_only_matching_alias() {
        use datafusion::arrow::datatypes::{DataType, Field, Schema};
        ...
        let key_a = SchemaCacheKey {
            alias: "source_a".to_string(),
            generation: 1,
            table_name: "orders".to_string(),
        };
        let key_b = SchemaCacheKey {
            alias: "source_b".to_string(),
            generation: 1,
            table_name: "orders".to_string(),
        };

        cache_schema(&state, key_a.clone(), schema.clone());
        cache_schema(&state, key_b.clone(), schema.clone());
        assert_eq!(state.schema_cache.read().unwrap().len(), 2);

        clear_schema_cache_for_source(&state, "source_a");

        let cache = state.schema_cache.read().unwrap();
        assert_eq!(cache.len(), 1, "only source_a entry should be removed");
        assert!(cache.contains_key(&key_b));
        assert!(!cache.contains_key(&key_a));
    }

    #[test]
    fn redact_sensitive_text_empty_is_unchanged() {
        assert_eq!(redact_sensitive_text(""), "");
    }

    #[test]
    fn redact_sensitive_text_plain_text_is_unchanged() {
        let plain = "no credentials here at all";
        assert_eq!(redact_sensitive_text(plain), plain);
    }

    #[test]
    fn redact_sensitive_text_scrubs_multiple_patterns() {
        let input = "postgres://alice:s3cret@host/db password=hunter2 pwd=topsecret";
        let result = redact_sensitive_text(input);
        assert!(!result.contains("s3cret"), "DSN password leaked");
        assert!(!result.contains("hunter2"), "password= leaked");
        assert!(!result.contains("topsecret"), "pwd= leaked");
        assert!(result.contains("***"));
    }

    fn make_req(method: &str, params: Option<serde_json::Value>, id: u64) -> RpcRequest {
        RpcRequest {
            jsonrpc: "2.0".to_string(),
    ...
    fn fresh_state() -> DaemonState {
        DaemonState::new(Arc::new(QsqlEngine::new()))
    }

    #[tokio::test]
    async fn handle_request_ping_returns_pong() {
        let resp = handle_request(make_req("ping", None, 1), fresh_state()).await;
    ...
    #[tokio::test]
    async fn handle_request_version_returns_product_name() {
        let resp = handle_request(make_req("version", None, 2), fresh_state()).await;
        assert_eq!(resp.result.as_ref().unwrap()["product"], "QuiverSQL");
        assert!(resp.error.is_none());
    }

    #[tokio::test]
    async fn handle_request_diagnostics_returns_counters() {
        let resp = handle_request(make_req("diagnostics", None, 3), fresh_state()).await;
        assert!(resp.result.as_ref().unwrap()["counters"].is_object());
        assert!(resp.error.is_none());
    }

    #[tokio::test]
    async fn handle_request_unknown_method_returns_32601() {
        let resp = handle_request(make_req("no_such_method", None, 4), fresh_state()).await;
        assert!(resp.result.is_none());
        assert_eq!(resp.error.as_ref().unwrap()["code"], -32601);
    }

    #[tokio::test]
    async fn handle_request_list_sources_initially_empty() {
        let resp = handle_request(make_req("list_sources", None, 5), fresh_state()).await;
        assert_eq!(resp.result.as_ref().unwrap().as_array().unwrap().len(), 0);
        assert!(resp.error.is_none());
    }

    #[tokio::test]
    async fn handle_request_execute_invalid_params_returns_32602() {
        let params = serde_json::json!({"query": "SELECT 1"});
        let resp = handle_request(make_req("execute", Some(params), 6), fresh_state()).await;
        assert!(resp.result.is_none());
        assert_eq!(resp.error.as_ref().unwrap()["code"], -32602);
    }

    #[tokio::test]
    async fn handle_request_execute_simple_select() {
        let params = serde_json::json!({"sql": "SELECT 1 AS n"});
        let resp = handle_request(make_req("execute", Some(params), 7), fresh_state()).await;
        assert!(resp.error.is_none());
        assert!(resp.result.is_some());
    }

    #[tokio::test]
    async fn handle_request_execute_json_simple_select() {
        let params = serde_json::json!({"sql": "SELECT 42 AS n"});
        let resp = handle_request(make_req("execute_json", Some(params), 8), fresh_state()).await;
        assert!(resp.error.is_none());
        let arr = resp.result.as_ref().unwrap().as_array().unwrap();
        assert_eq!(arr[0]["n"], 42);
    }

    #[tokio::test]
    async fn handle_request_query_cancel_unknown_id_returns_false() {
        let params = serde_json::json!({"query_id": "q_ghost"});
        let resp = handle_request(make_req("query_cancel", Some(params), 9), fresh_state()).await;
        assert!(resp.error.is_none());
        assert_eq!(resp.result.as_ref().unwrap()["cancelled"], false);
    }

    #[tokio::test]
    async fn handle_request_query_page_unknown_id_returns_32004() {
        let params = serde_json::json!({"query_id": "q_missing", "page_index": 0});
        let resp = handle_request(make_req("query_page", Some(params), 10), fresh_state()).await;
        assert_eq!(resp.error.as_ref().unwrap()["code"], -32004);
    }

    #[tokio::test]
    async fn handle_request_remove_source_unknown_returns_removed_false() {
        let params = serde_json::json!({"name": "ghost_db"});
        let resp = handle_request(make_req("remove_source", Some(params), 11), fresh_state()).await;
        assert!(resp.error.is_none());
        assert_eq!(resp.result.as_ref().unwrap()["removed"], false);
    }

    #[tokio::test]
    async fn handle_request_get_source_metadata_unknown_returns_32004() {
        let params = serde_json::json!({"name": "ghost_db"});
        let resp = handle_request(
            make_req("get_source_metadata", Some(params), 12),
            fresh_state(),
        )
        .await;
        assert_eq!(resp.error.as_ref().unwrap()["code"], -32004);
    ...
    #[tokio::test]
    async fn handle_request_register_sqlite_then_query() {
        ...
        let q = handle_request(make_req("query_start", Some(q_params), 21), state).await;
        assert!(q.error.is_none(), "query failed: {:?}", q.error);
        assert_eq!(q.result.as_ref().unwrap()["data"][0]["name"], "Sprocket");

        let _ = std::fs::remove_file(path);
    }

    #[tokio::test]
    async fn handle_request_list_source_tables_unknown_returns_32004() {
        let params = serde_json::json!({"name": "no_such_db"});
        let resp = handle_request(
            make_req("list_source_tables", Some(params), 22),
            fresh_state(),
        )
        .await;
        assert_eq!(resp.error.as_ref().unwrap()["code"], -32004);
    ...
    #[tokio::test]
    async fn handle_request_get_lineage_simple_query() {
        let params = serde_json::json!({"sql": "SELECT 1 AS n"});
        let resp = handle_request(make_req("get_lineage", Some(params), 23), fresh_state()).await;
        assert!(resp.error.is_none(), "get_lineage failed: {:?}", resp.error);
        assert!(resp.result.is_some());
    }

    #[tokio::test]
    async fn handle_request_register_file_then_list_sources() {
        use std::io::Write;
        use std::time::{SystemTime, UNIX_EPOCH};
        ...
        let sources = list.result.as_ref().unwrap().as_array().unwrap();
