use dql_core::engine::DqlEngine;
use dql_connectors::RemoteConnector;
use dql_connectors::sqlite::{SqliteConnector, SqliteTableProvider};
use std::io::Write;
use std::sync::Arc;

fn create_temp_csv() -> String {
    let path = std::env::temp_dir().join("test_federated_users.csv");
...
async fn create_temp_sqlite() -> String {
    let path = std::env::temp_dir().join("test_federated_salaries.db");
    let _ = std::fs::remove_file(&path);

    let connector = SqliteConnector::new(path.to_str().unwrap());
    // Create table and insert rows using our connector execution path!
    connector.execute_query("CREATE TABLE compensation (employee_id INTEGER, salary REAL)").await.unwrap();
    connector.execute_query("INSERT INTO compensation (employee_id, salary) VALUES (1, 120000.0) ").await.unwrap();
    connector.execute_query("INSERT INTO compensation (employee_id, salary) VALUES (2, 95000.0) ").await.unwrap();
    connector.execute_query("INSERT INTO compensation (employee_id, salary) VALUES (3, 80000.0) ").await.unwrap();
...
async fn test_federated_cross_source_join() {
    ...
    engine.register_file("users", &csv_path, "csv").await.unwrap();
