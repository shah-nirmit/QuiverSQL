...
- [x] EXPLAIN ANALYZE (opt-in via `qsql.explainAnalyzeEnabled`) with per-operator runtime metrics overlay
- [x] Full-scan warning and pushdown-reason annotations on `TableScan` plan-graph nodes
...
- [x] Column-level lineage: per-SELECT-list output columns, join conditions, aggregates, alias map
...
| Phase 10 | Complete | Rich explain, lineage, and performance visibility: typed `QueryLineage` walker (output columns + joins + aggregates + alias map), `EXPLAIN ANALYZE` opt-in (`analyze: Option<bool>`) with per-operator runtime metrics under the existing scan-guard envelope, `is_full_scan` + `pushdown_reason` attribute stamping on `TableScan` nodes, four-section lineage tree view in VS Code, plan-graph `Metrics` overlay toggle, third `🔍 Explain (ANALYZE)` CodeLens gated behind `qsql.explainAnalyzeEnabled`. |
| Phase 11 | Planned | Aggregate pushdown, cost-aware federated optimizer, packaging + release artifacts, benchmark report gates, and installer smoke tests. |
